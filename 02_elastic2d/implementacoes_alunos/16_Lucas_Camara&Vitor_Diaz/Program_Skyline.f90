...
!
!     Armazenamento da matriz de rigidez: Skyline (perfil de coluna)
!
!     >>> VERSAO FORTRAN 90 <<<
!     Conversao de sintaxe (fixed-form F77 -> free-form F90) do
!     programa original "programMEF.f". NENHUMA mudanca de logica,
!     algoritmo, formato de entrada/saida ou armazenamento (skyline)
!     foi feita. Mudancas sao puramente de forma:
!       - Forma livre (sem colunas fixas), continuacao com '&'
!       - Comentarios com '!'
!       - implicit none adicionado em todas as rotinas
!       - end program/subroutine/function nomeados
!     COMMON, EQUIVALENCE, GOTO (incluindo goto computado) e os
!     rotulos numericos foram mantidos tal como no original, pois
!     sao construcoes validas em Fortran 90 e alterar isso mudaria
!     a estrutura do codigo sem necessidade.
! ======================================================================
      program mef
      implicit none
      integer, parameter :: npos = 10000000
      integer :: m(npos)
      integer :: max
      common m
      common /size/ max
...
! ======================================================================
      ...
      integer :: nen1, i1, i2, i3, i4, i5, i6, i7, i8, i9, i10
      integer :: i11, i12, i13, i14, i15, i16, i17, i18, i19
      integer :: nst, neq, ncs, nvtk
...
!
!     Alocacao de memoria para jdiag e am:
!
!      i4    i5    i6    i7       i8
!      ------------------------------
!      |  x  |  f  |  u  | jdiag  |
...
!
      ...
      call mem(i8)
!
!     Perfil (skyline) da matriz de rigidez:
!
      call cpu_time(t0)
...
!
!     Alocacao de memoria para arrays locais e matriz global:
!
!      i7     i8   i9     i10    i11    i12   i13   i14
!      -----------------------------------------------
!      | jdiag | am |  xl  |  ul  |  fl  | sl  | ld  |
!      -----------------------------------------------
!
      nst = nen*ndf
      i9  =  i8  + ncs
...
!
!     Montagem da matriz de rigidez global K e correcao do vetor f:
...
!
      ...
      t_solver = t1 - t0
!
!     Relatorio de desempenho (tempo e memoria) - ARMAZENAMENTO SKYLINE:
!       Memoria da matriz:
!         - ncs valores real*8 (8 bytes cada) = vetor am (coeficientes)
!         - neq valores integer*4 (4 bytes cada) = vetor jdiag (ponteiros)
!
      mem_reais    = dble(ncs)
      mem_inteiros = dble(neq)
      mem_bytes = mem_reais * 8.d0 + mem_inteiros * 4.d0
      mem_kb    = mem_bytes / 1024.d0
      mem_mb    = mem_kb   / 1024.d0
      print*, '============================================='
      print*, ' RELATORIO DE DESEMPENHO - SKYLINE'
      print*, '============================================='
      print*, ' Numero de equacoes (neq)      :', neq
      print*, ' Tamanho do perfil (ncs)       :', ncs
      print*, ' --- Memoria da matriz K ---'
      print*, ' Reais real*8  : ncs =', ncs, 'x 8 bytes =', &
              nint(mem_reais*8.d0), 'bytes'
      print*, ' Inteiros int*4: neq =', neq, 'x 4 bytes =', &
              nint(mem_inteiros*4.d0), 'bytes'
      ...
      write(*,'(a,f12.6,a)') '                                  ', &
              mem_mb, ' MB'
      print*, ' --- Tempo ---'
      print*, ' Tempo fase simbolica  (profil):', t_simbolico, 's'
      print*, ' Tempo montagem        (pform) :', t_montagem,  's'
      print*, ' Tempo solver PCG      (pcg)   :', t_solver,    's'
...
!     Calculo do perfil (skyline) da matriz de rigidez.
!     Para cada coluna j, determina a altura efetiva (numero de
!     coeficientes nao-nulos acima da diagonal), com base na
!     conectividade dos elementos.
!     Preenche jdiag(i) = posicao da diagonal do coeficiente i
!     no vetor de armazenamento skyline.
! ======================================================================
      implicit none
      integer :: numel,nen,ndf,neq,ncs,ix(nen+1,*),id(ndf,*),jdiag(*)
      integer :: nel,i,j,k,l,noi,noj,kk,ll,m
      call mzero(jdiag,1,neq)
!
!     Alturas de coluna: para cada par de equacoes (kk, ll)
!     pertencentes ao mesmo elemento, a altura da coluna
!     max(kk,ll) deve incluir o coeficiente |kk-ll|.
!
      do 400 nel = 1, numel
      do 400 i = 1, nen
        noi = ix(i,nel)
  ...
  400 continue
!
!     Ponteiros da diagonal: converte alturas em posicoes absolutas.
!     jdiag(i) = jdiag(i-1) + altura(i) + 1
!
      ncs = 1
      jdiag(1) = 1
      if (neq .eq. 1) return
...
!     Loop nos elementos: monta a matriz de rigidez global e
!     corrige o vetor de forcas para deslocamentos prescritos.
...
!
!       Acumula contribuicoes local -> global:
!
...
!     Armazenamento skyline: apenas a parte triangular inferior
!     de cada coluna e armazenada.
...
!
        ...
        if (.not. bfl) goto 200
!
!       Montagem na matriz de rigidez (parte triangular inferior):
!       l = offset base da coluna k no vetor skyline
!
        l = jdiag(k) - k
        do 100 i = 1, nst
          m = ld(i)
...
!
!     Parametros:
!       a    = matriz de rigidez (formato skyline)
!       b    = vetor de forcas (entrada) / deslocamentos (saida)
!       jdiag= ponteiros do skyline
...
! ======================================================================
      ...
      maxit = 1000
!
!     Precondicionador diagonal: prec(i) = K(i,i) = a(jdiag(i))
!
      do 10 i = 1, neq
        prec(i) = a(jdiag(i))
...
!     Produto matriz-vetor para matriz simetrica no formato skyline.
!     Calcula: y = K * x
!
!     Aproveita a simetria: percorre apenas a parte armazenada
!     (triangulo inferior de cada coluna) e contribui tanto para
!     y(i) quanto para y(i-l), explorando K(i,i-l) = K(i-l,i).
! ======================================================================
      implicit none
      integer :: neq,i,j,l,m
      integer :: jdiag(*)
   ...
   50 continue
      do 200 i = 1, neq
!
!       Produto diagonal: y(i) += K(i,i) * x(i)
!
        j    = jdiag(i)
        y(i) = a(j)*x(i)
!
!       Produtos fora da diagonal (explorando simetria de K):
!       K(i, i-l) = a(j-l) contribui para y(i) e y(i-l)
!
        if (i .le. 1) goto 200
